...
本地访问 `/review`。公开复盘读取 `public/replays/` 中经过验证的历史记录，不调用 Jev API。包含正式评测中 716 个开局记录（708 手有效样本、2 手所在组未完成、6 手连接中断）、42 手试跑和 32 条诊断题，共 790 条。另 4 手在组中断后未开始，没有虚构回放。

支持按手逐步回放、自动播放、问题跳转、条件筛选、底牌搜索、查看对手底牌，以及检查模型当时的真实请求与返回。公开数据不包含认证头、密钥或未发牌堆。问题注释是离线分析，不是 Jev 的自述推理。

- 确定错误：合法动作或穷举未知对手底牌所证明的错误。
- 赔率问题：仅在对已知随机策略的单挑终局动作中，模拟权益与跟注赔率矛盾；使用 16,000 次模拟、99% 模拟误差区间和至少 1 bb 的保守差距。范围假设不用于 MC56 或跟注策略。
- 大额投入：仅提示复核，不自动判错。没有标注也不等于最优。

有本地原始日志时可运行 `npm run replay:build` 重建。该过程离线运行，不消耗模型额度。公开静态数据随代码保存，其他人克隆仓库后无需密钥即可查看复盘。
